...
import { Calendar, DollarSign, Package, Scissors, Users, UserCircle } from "lucide-react";
  ...
  return (
    <div className="p-6 max-w-6xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <Stat label="Today" value={todayCount} icon={<Calendar className="w-4 h-4" />} />
        <Stat label="Next 7 days" value={weekCount} icon={<Calendar className="w-4 h-4" />} />
        <Stat label="Staff" value={staffCount} icon={<Users className="w-4 h-4" />} />
        <Stat label="Customers" value={customersCount} icon={<UserCircle className="w-4 h-4" />} />
      </div>

      <Card className="mb-6">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg flex items-center gap-2"><DollarSign className="w-4 h-4" /> Projected revenue (next 7 days)</CardTitle>
                ...
                <li key={b.id} className="py-2 flex items-center justify-between text-sm">
                  <div>
                    ...
                    <div className="text-muted-foreground">
                      {b.service.name}{b.staff && ` · ${b.staff.name}`} · {format(b.startTime, "EEE MMM d, h:mm a")}
                    </div>
                  </div>
