...
export function CalendarView({
  view, baseDateISO, staff, hours, bookings, blocks, currency, defaultLocale, timezone,
}: {
  view: "day" | "week";
  baseDateISO: string;
  staff: Staff[];
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        <Button variant="outline" size="sm" onClick={() => navigate(0)}>{t("today")}</Button>
        <Button variant="ghost" size="icon" onClick={() => navigate(-1)} aria-label={t("previous")}><ChevronLeft className="w-4 h-4" /></Button>
        <Button variant="ghost" size="icon" onClick={() => navigate(1)} aria-label={t("next")}><ChevronRight className="w-4 h-4" /></Button>
        <div className="font-display text-xl ms-2">
          {view === "day"
            ? formatInTenantTz(baseDate, tz, "dateLong")
            : t("weekRange", {
                start: formatInTenantTz(startOfWeek(baseDate, { weekStartsOn: 1 }), tz, "dateMedium"),
                end: formatInTenantTz(addDays(startOfWeek(baseDate, { weekStartsOn: 1 }), 6), tz, "dateMedium"),
              })}
        </div>
        <div className="ml-auto flex gap-1 border border-border rounded-md p-0.5">
          <Button variant={view === "day" ? "default" : "ghost"} size="sm" onClick={() => setView("day")}>{t("viewDay")}</Button>
          <Button variant={view === "week" ? "default" : "ghost"} size="sm" onClick={() => setView("week")}>{t("viewWeek")}</Button>
        </div>
      </div>
          ...
          {selected && (
            <>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-2"><CalendarIcon className="w-3.5 h-3.5 text-muted-foreground" /> {formatInTenantTz(selected.startTime, tz, "short")}</div>
                <div className="flex items-center gap-2"><Clock className="w-3.5 h-3.5 text-muted-foreground" />
                  {t("durationMinutes", { minutes: Math.round((parseISO(selected.endTime).getTime() - parseISO(selected.startTime).getTime()) / 60000) })}
                </div>
                {selected.staffName && (
                ...
                <div className="pt-2 flex items-center gap-2">
                  <Badge variant={selected.status === "confirmed" ? "success" : "secondary"}>{statusLabel(selected.status, t)}</Badge>
                  <Badge variant={
                    selected.paymentStatus === "paid" ? "success" :
                    selected.paymentStatus === "refunded" ? "outline" :
                    selected.paymentStatus === "pending" ? "secondary" :
                    "outline"
                  }>{paymentLabel(selected.paymentStatus, t)}</Badge>
...
function DayGrid({
  day, staff, bookings, blocks, rangeStartMin, gridHeight, hourMarks, bookingStyle, onSelect, t, tz,
}: any) {
  ...
  const cols: Staff[] = staff.length > 0 ? staff : [{ id: "_", name: t("unassigned"), color: "#888" }];
...
function WeekGrid({
  days, staff, bookings, rangeStartMin, gridHeight, hourMarks, bookingStyle, onSelect, t, tz,
}: any) {
  return (
    <div className="flex" style={{ minHeight: gridHeight + 40 }}>
      <TimeAxis hourMarks={hourMarks} rangeStartMin={rangeStartMin} gridHeight={gridHeight} />
      <div className="flex-1 grid" style={{ gridTemplateColumns: `repeat(7, minmax(140px, 1fr))` }}>
        {days.map((d: Date) => {
          ...
          return (
            <div key={d.toISOString()} className="border-e border-border/40 relative last:border-e-0">
              <div className="h-10 border-b border-border/60 px-2 flex flex-col items-center justify-center text-xs">
                <div className="uppercase tracking-wider text-muted-foreground">{formatInTenantTz(d, tz, "weekdayShort")}</div>
                <div className={cn("font-display text-base", isSameDay(d, new Date()) && "text-accent font-medium")}>{formatInTenantTz(d, tz, "dayNum")}</div>
              </div>
                ...
                {dayBookings.map((b) => {
                  ...
                  return (
                    <button
                      key={b.id}
                      onClick={() => onSelect(b)}
                      className="absolute inset-x-1 rounded text-[11px] p-1 text-start overflow-hidden hover:shadow-md transition"
                      style={{ top, height, background: color + "22", borderLeft: `3px solid ${color}` }}
                      title={`${b.customerName} — ${b.serviceName}`}
                    >
                      <div className="font-medium truncate">{formatInTenantTz(b.startTime, tz, "time")} {b.customerName}</div>
...
function TimeAxis({ hourMarks, rangeStartMin, gridHeight }: any) {
  return (
    <div className="w-14 border-e border-border/60 bg-secondary/20 shrink-0">
      <div className="h-10 border-b border-border/60" />
      <div className="relative" style={{ height: gridHeight }}>
        {hourMarks.map((m: any) => (
          <div
            key={m.min}
            className="absolute -translate-y-1/2 text-[10px] text-muted-foreground ps-2"
    ...
    <>
      {hourMarks.map((m: any) => {
        ...
        return (
          <div
            key={m.min}
            className={cn("absolute start-0 end-0 border-t pointer-events-none", isHour ? "border-border/50" : "border-border/20")}
            style={{ top: (m.min - rangeStartMin) * PIXEL_PER_MIN }}
          />
        );
      })}
...
function StaffColumn({ staff, bookings, blocks, hourMarks, rangeStartMin, gridHeight, bookingStyle, onSelect, t, tz }: any) {
  return (
    <div className="border-e border-border/40 relative last:border-e-0">
        ...
        {bookings.map((b: Booking) => {
          ...
          return (
            <button
              key={b.id}
              onClick={() => onSelect(b)}
              className="absolute inset-x-1 rounded text-xs p-2 text-start overflow-hidden hover:shadow-md transition"
              style={{ top, height, background: color + "22", borderLeft: `3px solid ${color}` }}
            >
              <div className="font-medium truncate">{formatInTenantTz(b.startTime, tz, "time")} · {b.customerName}</div>
              <div className="truncate opacity-80">{b.serviceName}</div>
              {b.status === "pending" && <div className="text-[10px] uppercase tracking-wider opacity-70 mt-0.5">{t("pendingPayment")}</div>}
            </button>
          );
        })}
      </div>
...
function paymentLabel(ps: string, t: (k: string) => string): string {
  switch (ps) {
    case "none":     return t("paymentNone");
